...
#pragma target 3.0
	...
	void surf(Input IN, inout SurfaceOutputStandard o) {
		// using uv resulted in stripes appearing
		float3 localPos = IN.worldPos - mul(unity_ObjectToWorld, float4(0, 0, 0, 1)).xyz;
		float2 xzPos = float2(localPos.x, localPos.z);
		o.Albedo = 1;
		float noiseValue = voronoiNoise(float3(_Time[0]* _Speed, xzPos*_Scale));
		float transformed = pow(noiseValue, _Sharpness)*_Opacity;
		o.Alpha = transformed;
		o.Emission = transformed;
	}
	ENDCG
	}
		Fallback "Diffuse"
